...
class abstract_page {
     ...
     */
    function has_cap() {
        return has_capability($this->get_cap(), flexible_model::get_instance()->get_context());
    }

    /** Requires capabilities to view, used in has_ability_to_view
     */
    function require_cap() {
        return require_capability($this->get_cap(), flexible_model::get_instance()->get_context());
    }
...
class assignee_choose_form extends moodleform {
    function definition() {
        ...
        $recs = $DB->get_records('flexible_assignee',array('flexibleid' => $flexibleid));
        foreach ($recs as $rec) {
            $lastattemptid = flexible_model::get_instance()->get_last_attempt_id($rec->id);
        ...
        $mform->addElement('select', 'assigneeid', get_string('assignee', 'flexible'),$recs);
...
class attempt_choose_form extends moodleform {
    function definition() {
        ...
        $instance = $this->_customdata;
        $flexibleid = flexible_model::get_instance()->get_flexible()->id;
        $attempts = $DB->get_records('flexible_attempts', array('assigneeid' => $instance['assigneeid']),'id DESC');
        foreach ($attempts as $attempt) {
            $attempts[$attempt->id] = get_string('attempt', 'flexible') . $attempt->attemptnumber . ':' . userdate($attempt->attemptdate);
        }
        $mform->addElement('select', 'attemptid', get_string('attempt', 'flexible'),$attempts);
        $mform->addElement('submit', 'submit', 'go');
