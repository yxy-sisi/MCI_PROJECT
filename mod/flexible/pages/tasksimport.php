...
class tasksimport_page extends abstract_page {
    ...
    function pre_view() {
        ...
        $this->mform = new tasksimport_form(
            ...
            array(
                ...
                'options' => auditor_sync::get_instance()->get_possible_kc_fields($flexibleid))
        );
    }
    ...
    function view() {
        ...
        if (count($this->taskrecords) > 0) {
            foreach ($this->taskrecords as $taskrecord) {
                ...
                echo get_string('taskimported', 'flexible');
        ...
        if ($this->mform->get_data()) {
            ...
            if ($data->submitbutton == get_string('sync', 'flexible')) {
            ...
            if (($error == false)) {
                ...
                if (!($result = mysql_query($sql))) {
                    ...
                    $variants = $DB->get_records('flexible_variants', array('fieldid' => $data->kcfield), 'sortorder');
                    $levels = array();
                    ...
                    if (!($complexities = mysql_query($sql))) {
                        ...
                        while ($variant = mysql_fetch_assoc($result)) {
                            ...
                            $taskrecord = new stdClass();
                            $taskrecord->name = get_string('defaulttaskname', 'flexible') .' №' . $variant['num'];
                                ...
                                while ($modification = mysql_fetch_assoc($modifications)) {
                                    ...
                                    foreach ($newcomplexities as $complexity) {
                                        if ($modification['kc'] >= $complexity['kcmin']
                                            ...
                                            // Узнать id этого уровня сложности в таблице flexible_variants
            ...
            if (!($result = mysql_query($sql))) {
                ...
                while ($actualtask = mysql_fetch_assoc($result)) {
                        ...
                        foreach ($taskstosync as $tasktosync) {
                            ...
                            if ($flexibletask) {
                                if ($flexibletask->description != $actualtask['description']) {
                                    ...
                                    //echo '<br/>UPDATE RECORD flexible_tasks ',print_r($flexibletask);
                    ...
                    // Если такого задания в moodle нет, добавить его
                        ...
                        $taskrecord = new stdClass();
                        $taskrecord->name = get_string('defaulttaskname', 'flexible') .' №' . $actualtask['num'];
    ...
    private function show_preview($tasks, $kcfieldname, $levels) {
        ...
        $table = new flexible_table('mod-flexible-task-import');
        ...
        $table->set_attribute('class', 'flexible-table task-import');
        ...
        $model = flexible_model::get_instance();
        $flexibleid = $model->get_flexible()->id;
        echo '<input type="hidden" name="flexibleid" value="'.$flexibleid.'"/>';
...
class tasksimport_form extends moodleform {
    function definition(){
        ...
        $mform->addElement('text','server',get_string('server','flexible'),array('size'=>45));
        $mform->setDefault('server', $config['server']);
        $mform->addRule('server', null, 'required', null, 'client');

        $mform->addElement('text','database',get_string('database','flexible'),array('size'=>45));
        $mform->setDefault('database', $config['database']);
        $mform->addRule('database', null, 'required', null, 'client');

        $mform->addElement('text','dbuser',get_string('dbuser','flexible'),array('size'=>45));
        $mform->setDefault('dbuser', $config['dbuser']);
        $mform->addRule('dbuser', null, 'required', null, 'client');

        $mform->addElement('password','dbpass',get_string('dbpass','flexible'),array('size'=>45));
        ...
        $this->add_action_buttons(true, get_string('import', 'flexible'));
        $mform->addElement('submit', 'submitbutton', get_string('sync', 'flexible'));
    }
}